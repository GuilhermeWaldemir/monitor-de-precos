...
## 14. O acesso ao Mercado Livre morria a cada 6 horas
*2026-09-17*

- **Problema:** depois de conectar a conta, o Mercado Livre não devolvia `refresh_token`, então o acesso vencia em 6 horas e a verificação agendada das 12:30 falharia todo dia. A documentação diz que o `refresh_token` só vem com a permissão **`offline_access`**, mas essa opção não aparecia no formulário da aplicação (a tela de "Permissões" só tem Leitura/Escrita por recurso).
- **Processo:** primeiro tentei pedir o escopo na própria autorização (`scope=offline_access read`) — o Mercado Livre aceitou a autorização, mas continuou sem mandar o `refresh_token`. Em vez de insistir, testei o outro fluxo do OAuth: **`client_credentials`**, em que o aplicativo pede um token com as próprias credenciais, sem usuário nenhum. A API devolveu token válido por 6 horas, com escopo `offline_access read`, e as chamadas de produto responderam **200**.
